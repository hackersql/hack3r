...
trait TraversableOnce[+A] extends Any with GenTraversableOnce[A] {
   ...
   *  对此$coll的所有元素应用二进制运算符
   *  going left to right.从左到右
   *  $willNotTerminateInf
   *  $orderDependentFold
   *
   *  @param  op    the binary operator.二进制运算符
   *  @tparam  B    the result type of the binary operator.二进制运算符的结果类型
   *  @return  the result of inserting `op` between consecutive elements of this $coll,
   *           在这个$coll的连续元素之间插入`op`的结果
   *           going left to right:
   *           {{{
   *             op( op( ... op(x_1, x_2) ..., x_{n-1}), x_n)
   *           }}}
   *           where `x,,1,,, ..., x,,n,,` are the elements of this $coll.
   *           其中`x,,1,,, ..., x,,n,,`是这个$coll的元素
   *  @throws UnsupportedOperationException if this $coll is empty.   
   *          如果$coll为空，则抛出一个UnsupportedOperationException。*/
