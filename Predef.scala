...
object Predef extends LowPriorityImplicits with DeprecatedPredef {
   ...
   *  测试表达式，如果为false，则抛出“IllegalArgumentException”。
   *  This method is similar类似 to `assert`, but blames the caller of the method
   *  for violating违反 the condition.
   *  此方法类似于 "断言", 但将其归咎于该方法的调用方违反了条件。
   *
   *  @param requirement   the expression to test
   *  @group assertions 断言
   *  require 方法带一个布尔型参数。如果传入的值为真，require 将正常返回。
   *  反之，require将通过抛出IllegalArgumentException 来阻止对象被构造。 
   */
